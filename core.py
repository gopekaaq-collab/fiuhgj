...
def cantor_login(hs: HumanSession, party_id, signing_key):
...
def vector_login(hs: HumanSession, canton_address):
    nonce = hs.get(config.NONCE_URL, use_cantor=False).json()["nonce"]
...
def execute_transaction(hs: HumanSession, cantor_token, prepared, signing_key, mode, order_id):
    before_canton, before_ceth = get_balance(hs, cantor_token)
    ...
    for _ in range(300):
        time.sleep(2)
        after_canton, after_ceth = get_balance(hs, cantor_token)
...
def trader_cycle(hs: HumanSession, mnemonic, party_id, signing_key, status_callback=None):
    ...
    current_canton, current_ceth = get_balance(hs, cantor_token)
    ...
    if current_canton < 26:
        ...
        if result in ("SERVICE_DOWN", "SKIP_CYCLE", None):
            ...
            return False
        time.sleep(1800)
        initial_cc, initial_ceth = get_balance(hs, cantor_token)
    else:
        initial_cc, initial_ceth = current_canton, current_ceth

    # SELL
    log("SELL phase started")
    current_canton, _ = get_balance(hs, cantor_token)
    if current_canton <= 26:
    ...
    _, current_ceth = get_balance(hs, cantor_token)
    ...
    final_cc, final_ceth = get_balance(hs, cantor_token)
...
def run_trader_worker(mnemonic, proxy_list, status_callback=None, stop_event=None):
    ...
    try:
        ...
        if not party_id:
            if status_callback:
                status_callback("ERROR: Failed to get party_id")
            return
        signing_key, _ = build_keypair_from_mnemonic(mnemonic)
        cycle_count = 0
        while True:
            if stop_event and stop_event.is_set():
                break
            if status_callback:
                status_callback(f"Starting trade cycle #{cycle_count + 1}")
            success = trader_cycle(hs, mnemonic, party_id, signing_key, status_callback)
            if not success:
                if status_callback:
                    status_callback("Cycle failed. Retrying in 5 minutes...")
                time.sleep(300)
            else:
                cycle_count += 1
