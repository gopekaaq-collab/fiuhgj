...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data.startswith("trader_wallet_"):
        idx = int(data.split("_")[-1])
        await launch_trader(query, idx)
...
async def start_trader_menu(query):
    mnemonics = bot_state["mnemonics"]
    if not mnemonics:
        await query.edit_message_text(
            luxury_box("ERROR", ["No wallets available."]),
            reply_markup=back_menu()
        )
        return

    buttons = []
    for i, _ in enumerate(mnemonics):
        status = "RUNNING" if i in bot_state["trader_threads"] else "IDLE"
        buttons.append([InlineKeyboardButton(f"Wallet {i+1} [{status}]", callback_data=f"trader_wallet_{i}")])
    buttons.append([InlineKeyboardButton("Back to Main Menu", callback_data="back_main")])

    await query.edit_message_text(
        luxury_box("SELECT WALLET", ["Choose wallet to start trading:"]),
        reply_markup=InlineKeyboardMarkup(buttons)
    )


async def launch_trader(query, idx):
    if idx in bot_state["trader_threads"] and bot_state["trader_threads"][idx].is_alive():
        await query.edit_message_text(
            luxury_box("TRADER", [f"Wallet {idx+1} is already running."]),
            reply_markup=back_menu()
        )
        return

    stop_event = threading.Event()
    bot_state["trader_stop_events"][idx] = stop_event

    def status_cb(msg):
        add_log(f"[Wallet {idx+1}] {msg}")

    mnemonic = bot_state["mnemonics"][idx]
    proxies = bot_state["proxies"]

    t = threading.Thread(
        target=core.run_trader_worker,
        args=(mnemonic, proxies, status_cb, stop_event),
        daemon=True
    )
    t.start()
    bot_state["trader_threads"][idx] = t
    add_log(f"Trader started for wallet {idx+1}")
...
async def stop_all_traders(query):
    count = 0
    for idx, ev in list(bot_state["trader_stop_events"].items()):
        ev.set()
        count += 1
    bot_state["trader_threads"].clear()
    bot_state["trader_stop_events"].clear()
